  ...
  <div class="flex">
    ...
    <div class="p-4 el-card flex-1 h-full m-l-3 flex flex-column">
      <div class="flex align-items-center justify-content-between">
        ...
        <div>
          <span v-if="errorShow" class="error m-r-8">入库物料清单不能为空</span>
          <el-button
            :type="'primary'"
            @click="addMaterial"
...
const validateInStockNoFn = async (rule: any, value: any, callback: any) => {
  if (value == "") return callback(new Error("不能为空"));
  const params: any = { inStockNo: form.value.inStockNo };
  if (form.value.id) params.id = form.value.id;
  const res: any = await validateInStockNo(params);
  if (!res.data) {
    callback(new Error("入库单号不能重复"));
  } else {
    callback();
  }
};
const rules = reactive<FormRules>({
  inStockNo: [{ validator: validateInStockNoFn as any, trigger: "blur" }],
...
const columns = computed(() => {
  const result: any[] = [
    { prop: "index", label: "序号", type: 1, width: 15 },
    ...
    materialIds.value =
      res.data?.map((item: any) => {
        ...
        detailMap.value.set(materialId, {
          ...item,
          expectedCount: item.applyCount,
          perPrice: Number(item?.material?.costPrice ?? 0),
        });
        return materialId;
      }) ?? [];
    refreshTable();
  ...
  try {
    ...
    if (valid) {
      ...
      const params: any = {
        ...form.value,
        inStockTime:
          typeof form.value.inStockTime === "string"
            ? new Date(form.value.inStockTime).getTime()
            : form.value.inStockTime.getTime(),
      };
