                ...
                <template #operate="scope">
                  <div class="flex">
                    ...
                    <IconSvg
                      ...
                      v-if="
                        enableEdit &&
                        scope.scope.row.hasInStockAuth &&
...
import Abandon from "@pages/stockInManagement/stockIn/abandon.svg";
import StockIn from "@pages/stockInManagement/stockIn/stockIn.svg";
...
const abandon = (row: any) => {
  ElMessageBox.confirm("确定要废弃吗？", "入库单", {
    ...
    .then(async () => {
      ...
      delete params["files"];
      const res: any = await editInStock(params);
      if (res.code) {
        ElMessage({
          type: "success",
          message: "废弃成功",
...
const stockInDetail = (row: any) => {
  ElMessageBox.confirm("确定要入库吗？", "入库单", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
