          ...
          <el-card v-loading="loading" shadow="never" class="search-wrapper">
            <div class="flex">
              <el-form
                ...
                :model="searchData"
              >
                <el-form-item label="领用单号" prop="applyNo">
                  <el-input
                ...
                <el-form-item prop="status" label="领用状态">
                  <el-select
                  ...
                  >
                    <el-option
                      v-for="item in outBoundApplyStatusListWithAll"
                      :key="item.id"
                      :label="item.name"
                      :value="item.id"
                    />
            ...
            <div class="table-wrap">
              ...
              >
                ...
                <template #status="scope">
                  <el-tag type="primary">
                    {{
                      getStatus(scope.scope.row.status, OutBoundApplyStatusList)
                    }}
                  </el-tag>
                </template>
...
import {
  queryOutBoundApplyConditions,
  deleteOutBoundApply,
  findOutBoundApplyPage,
  OutBoundApply,
  editOutBoundApply,
  OutBoundApplyStatusList,
  AuditStatusList,
} from "@pages/outBoundApplyManagement/api/outBoundApply";
import {
  getStockListByIds,
  getStockList,
  Stock,
...
const outBoundApplyStatusListWithAll = [
  { id: -1, name: "全部" },
  ...OutBoundApplyStatusList,
];
const treeRef = ref();
...
const columns = ref([
  ...
  { prop: "applyNo", label: "领用单号" },
  { prop: "applyTime", label: "领用时间" },
  { prop: "applyUserId", label: "领用人" },
...
const currentData = ref<OutBoundApply | null>(null);

const createRef = ref();
const tableData = ref<OutBoundApply[]>([]);

const searchFormRef = ref("searchFormRef");

const searchData = reactive<queryOutBoundApplyConditions>({
  applyNo: "",
  auditStatus: -1,
  status: -1,
...
function refreshTable() {
  loading.value = true;
  const params: PaginatedRequest<queryOutBoundApplyConditions> = {
...
const abandon = (row: any) => {
  ElMessageBox.confirm("确定要废弃吗？", "领用单", {
    ...
    .then(async () => {
      ...
      delete params["files"];
      const res: any = await editOutBoundApply(params);
      if (res.code) {
        ElMessage({
          type: "success",
          message: "废弃成功",
