          ...
          <el-card v-loading="loading" shadow="never" class="search-wrapper">
            <div class="flex">
              ...
              >
                ...
                <el-form-item prop="status" label="出库状态">
                  <el-select
                  ...
                  >
                    <el-option
                      v-for="item in outStockStatusListWithAll"
                      :key="item.id"
                      :label="item.name"
                      :value="item.id"
                    />
            ...
            <div class="table-wrap">
              ...
              >
                ...
                <template #status="scope">
                  <el-tag type="primary">
                    {{ getStatus(scope.scope.row.status, OutStockStatusList) }}
                  </el-tag>
                </template>
                ...
                <template #outStockTime="scope">
                  {{
                    scope.scope.row.outStockTime
                      ? new Date(scope.scope.row.outStockTime)
                          .toLocaleString()
                          .replace(/\//g, "-")
                      : "-"
                  }}
                ...
                <template #operate="scope">
                  <div class="flex">
                    ...
                    <IconSvg
                      ...
                      v-if="
                        enableEdit &&
                        scope.scope.row.hasOutStockAuth &&
...
import {
  ...
  OutStockStatusList,
  AuditStatusList,
} from "@pages/stockOutManagement/api/outStock";
import {
  getStockListByIds,
  getStockList,
  Stock,
...
import { materialStockDetailSave } from "@pages/inventoryManagement/api/stockDetail";
import Abandon from "@pages/stockOutManagement/stockOut/abandon.svg";
import StockOut from "@pages/stockOutManagement/stockOut/stockOut.svg";
...
const outStockStatusListWithAll = [
  { id: -1, name: "全部" },
  ...OutStockStatusList,
];
const treeRef = ref();
...
const currentData = ref<OutStock | null>(null);

const createRef = ref();
const tableData = ref<OutStock[]>([]);

const searchFormRef = ref("searchFormRef");

const searchData = reactive<queryOutStockConditions>({
  auditStatus: -1,
  status: -1,
...
function refreshTable() {
  loading.value = true;
  const params: PaginatedRequest<queryOutStockConditions> = {
...
const abandon = (row: any) => {
  ElMessageBox.confirm("确定要废弃吗？", "出库单", {
    ...
    .then(async () => {
      ...
      delete params["files"];
      const res: any = await editOutStock(params);
      if (res.code) {
        ElMessage({
          type: "success",
          message: "废弃成功",
...
const stockOutDetail = (row: any) => {
  ElMessageBox.confirm("确定要出库吗？", "出库单", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
    .then(async () => {
      const res: any = await materialStockDetailSave({
        orderId: row.id,
        type: 2,
      });
      if (res.code) {
