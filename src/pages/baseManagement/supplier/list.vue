        ...
        <div class="table-wrap">
          ...
          >
            <template #status="scope">
              <el-tag
                :type="scope.scope.row.status === 1 ? 'primary' : 'warning'"
                >{{
                  getName(scope.scope.row.status, statusOptions)?.name
                }}</el-tag
              >
            </template>
            <template #creditLevel="scope">
              <el-tag
                ...
                >{{
                  getName(scope.scope.row.creditLevel, creditLevelOptions)
                }}</el-tag
              >
            </template>
...
const getName = (id: string, options: any[]) => {
  const result = options.find((item: any) => item.id == id);
  return result ?? "无";
};
onMounted(async () => {
  refreshTable();
