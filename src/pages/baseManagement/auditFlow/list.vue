                ...
                <template #operate="scope">
                  <div class="flex">
                    ...
                    <el-icon
                      ...
                      text
                      v-if="enableDelete && !scope.scope.row.enable"
                      @click="
                        remove(scope.scope.row.id, scope.scope.row.enable)
                      "
                    >
                      <Delete />
                    </el-icon>
...
const back = () => {
  if (processFlag.value == 2 && !stepRef.value.validate()) {
    ElMessage({
      type: "error",
      message: "审批步骤不能为空",
    });
...
const remove = async (id: string, enable: boolean) => {
  if (enable) {
    ElMessage({
      type: "error",
      message: "已启用的审批流程禁止删除",
    });
    return;
  }
