...
const validateCodeFn = async (rule: any, value: any, callback: any) => {
  if (value == "") return callback(new Error("不能为空"));
  const res: any = await validateCode(form.value);
  if (!res.data) {
...
const validateUsernameFn = async (rule: any, value: any, callback: any) => {
  if (value == "") return callback(new Error("不能为空"));
  const res: any = await validateUsername(form.value);
  if (!res.data) {
...
const rules = reactive<FormRules>({
  code: [{ validator: validateCodeFn as any, trigger: "blur" }],
  username: [
    {
      validator: validateUsernameFn as any,
      trigger: "blur",
    },
  ],
  ...
  phone: [
    { required: true, validator: validatePhone as any, trigger: "blur" },
    { pattern: /^1[3-9]\d{9}$/, message: "电话格式不正确" },
  ],
  email: [
    { required: true, validator: validateEmail as any, trigger: "blur" },
    { type: "email", message: "email填写格式不正确", trigger: "blur" },
  ],
});
