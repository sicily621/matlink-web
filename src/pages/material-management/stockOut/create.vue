<template>
  <div class="flex-1 flex-column h-full">
      ...
      <div class="d-flex h-full">
        <div class="fill-bg radius p-16 m-r-8">
          ...
          <el-form ref="formRef" :model="form" :rules="rules" label-position="right" class="d-flex relative m-r-10 m-t-20" :label-width="zrem(120)">
            ...
            <div class="d-flex" v-else>
              <div class="m-r-30">
                <el-form-item :label="i18nM('material.table.warehouseOutNumber')" prop="outStockNo">
                  <el-input
                    v-model="form.outStockNo"
                    class="w-200"
                    :placeholder="i18nM('material.placeholder.warehouseOutNumber')"
                    maxlength="32"
                    required
                    :disabled="onlyView"
                  >
                ...
                <el-form-item :label="i18nM('material.label.stockOutType')" prop="type">
                  <el-select
                    class="w-200"
                    :disabled="onlyView || !!form.resourceId || !!tableData.length"
                    v-model="form.type"
                    :placeholder="i18nM('material.placeholder.select')"
                  >
                    <el-option v-for="item of stockOutTypeOptions" :key="item.id" :value="item.id" :label="item.name"></el-option>
                  </el-select>
                </el-form-item>
                ...
                <el-form-item :label="i18nM('material.table.outStockTime')" prop="outStockTime">
                  <el-date-picker
                    :disabled="onlyView"
                ...
                <el-form-item :label="i18nM('material.table.outStockUser')" prop="outStockUserName"
                  ><el-input class="w-200" disabled v-model="form.outStockUserName" :placeholder="i18nM('material.placeholder.select')">
                    <template #append>
                      <el-button class="w-30 p-0" :icon="More" @click="openModal" />
                    </template>
                  </el-input>
                </el-form-item>
                ...
                <el-form-item :label="i18nM('material.label.directOut')" prop="isDirect">
        ...
        <div class="fill-bg radius flex-1 flex-column p-16">
          <div class="d-flex align-items-center justify-content-between h-32">
            ...
            <div>
              <span v-if="errorShow" class="error m-r-8">{{ i18nM("material.error.stockOutDetailCannotBeEmpty") }}</span>
              <el-button :type="'primary'" @click="addMaterial" v-if="!onlyView && (form.type === 3 || form.type === 4)">
    ...
    <zModel
      ref="orderRef"
      :style="{ width: '100%', height: '100%' }"
      :custom-model-title="form.type === 1 ? i18nM('material.title.materialOutBoundApply') : i18nM('material.title.warehouseReceipt')"
      :noConfirmBtnFlag="true"
      @confirm="() => {}"
    >
      <div class="flex-1 d-flex w-full h-full relative content-bg">
...
import { stockOut, saveOutStock, generateOutStockNo, deleteOutStockFile } from "@functions/serviceCenter/material-management/api/outstock";
import { saveOutStockDetail, deleteByOutStockId, getOutStockDetail } from "@functions/serviceCenter/material-management/api/outstockDetail";
...
import Material from "@functions/serviceCenter/material-management/stockOut/material.vue";
import Order from "@functions/serviceCenter/material-management/stockOut/resourceOrder.vue";
const props = defineProps<{ data: stockOut | null; projectId: string | number; stockId: string | number; onlyView: boolean }>();
...
const getMax = (row: any) => {
  if (form.value.type > 2) {
    ...
    return Math.min(row.expectedCount, row.useCount);
  }
};
const inputChange = (row: any) => {
  row.totalPrice = Number(Number(+row.perPrice * +row.actualCount).toFixed(2));
  row.outStockPrice = Number(Number(row.totalPrice - row.outStockPrice * +row.priceRatio).toFixed(2));
};
...
const remove = (file: any, index: number) => {
  ElMessageBox.confirm(i18nM("material.confirm.deleteFile"), i18nM("material.title.deleteFile"), {
    ...
    .then(() => {
      if (file.id) {
        deleteOutStockFile({ id: file.id }, (res: any) => {
          if (res.code) {
            ElMessage({
              type: "success",
...
let validatorResourceId = (rule: any, value: string, callback: Function) => {
  if (form.value.type === 3 || form.value.type === 4) callback();
  if (value.length === 0) {
...
const rules = reactive({
  outStockNo: [{ required: true, message: i18nM("material.error.cannotBeEmpty") }],
  type: [{ required: true, message: i18nM("material.error.cannotBeEmpty") }],
  outStockTime: [{ required: true, message: i18nM("material.error.cannotBeEmpty") }],
  outStockUserName: [{ required: true, message: i18nM("material.error.cannotBeEmpty") }],
...
const columns = computed(() => {
  const result: any[] = [
    ...
    { prop: "useCount", label: i18nM("material.table.useCount") },
    { prop: "expectedCount", label: i18nM("material.table.sourceQuantity") },
    { prop: "actualCount", label: i18nM("material.table.stockOutCount"), type: props.onlyView ? 0 : 8 },
    { prop: "costPrice", label: i18nM("material.table.costPrice") },
    { prop: "totalCostPrice", label: i18nM("material.table.totalCostPrice") },
    { prop: "perPrice", label: i18nM("material.table.stockOutUnitPrice"), type: props.onlyView ? 0 : 8 },
...
const disabledHours = () => {
  if (!form.value.outStockTime) return [];

  const selectedDate = new Date(form.value.outStockTime);
...
const outStockUserId = ref("");
const outStockUserName = ref("");
const currentChange = (row: any) => {
  outStockUserId.value = row.id;
  outStockUserName.value = row.name;
};
const modelRef = ref<any>(null);
const openModal = () => {
  modelRef.value.openCustomModel();
  nextTick(() => {
    const targetRow = userTableData.value.find((row: any) => row.id === form.value.outStockUserId);
...
const saveOrder = (data: any) => {
    ...
    getMaterialOutBoundApplyDetail({ applyId: form.value.resourceId }, (res: any) => {
      ...
      materialIds.value =
        res.data?.map((item: any) => {
          const { tradeId } = item;
          detailMap.value.set(tradeId, { ...item, expectedCount: item.applyCount, actualCount: item.applyCount });
          return tradeId;
        }) ?? [];
      refreshTable();
    });
  ...
  if (form.value.type == 2) {
    getInStockDetail({ inStockId: form.value.resourceId }, (res: any) => {
      detailMap.value.clear();
      materialIds.value =
        res.data?.map((item: any) => {
          const { tradeId } = item;
          detailMap.value.set(tradeId, { ...item, expectedCount: item.actualCount });
          return tradeId;
        }) ?? [];
      refreshTable();
    });
...
const savePerson = () => {
  form.value.outStockUserId = outStockUserId.value;
  form.value.outStockUserName = outStockUserName.value;
...
const confirmSave = (cb: Function) => {
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      const params: any = {
        ...form.value,
        outStockTime: new Date(form.value.outStockTime).getTime(),
      };
      delete params["outStockUserName"];
      ...
      saveOutStock(formData, (res: any) => {
        if (res.code) {
          deleteByOutStockId({ outStockId: res.data.id }, (delRes: any) => {
            if (delRes.code) {
              const formData: any = new FormData();
              tableData.value.map((item: any, index: number) => {
                const { tradeId, expectedCount, actualCount, perPrice, totalPrice, priceRatio, outStockPrice } = item;
                const result: any = {
                  projectId: props.projectId,
                  outStockId: res.data.id,
                  tradeId,
              ...
              saveOutStockDetail(formData, (saveRes: any) => {
                if (saveRes.code) {
                  ElMessage({
                    type: "success",
...
onMounted(() => {
  ...
  if (props?.data?.id) {
    ...
    getOutStockDetail({ outStockId: props.data.id }, (res: any) => {
      detailMap.value.clear();
      materialIds.value =
        res.data?.map((item: any) => {
          const { tradeId } = item;
    ...
    generateOutStockNo((res: any) => {
      form.value.outStockNo = res.data;
    });
  }
});
defineExpose({
