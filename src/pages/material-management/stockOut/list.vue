          ...
          <div class="d-flex flex-1 align-items-center justify-content-between">
            <div class="d-flex align-items-center">
              <div class="title d-flex w-150">
                ...
                <span>{{ i18nM("material.title.stockOutNote") }}</span>
              </div>
              ...
              <el-select class="select-item" v-model="businessStatus" :placeholder="i18nM('material.placeholder.select')" @change="refreshTable()">
            ...
            <div class="zc-base p-r-20 p-l-20 flex-column flex-1">
              <div ref="tableRef" class="flex-1 table-wrap">
                <baseTable :columns="columns" :table-data="tableData" :indexMethod="indexMethod(currentPage, pageSize)">
                  ...
                  <template #outStockStatus="scope">
                    {{ findNameById(businessStatusOptions, scope.scope.row.outStockStatus) }}
                  </template>
                  <template #status="scope">
                    {{ findNameById(reviewStatusOptions, scope.scope.row.status) }}
                  </template>
                  <template #outStockUserId="scope">
                    {{ userNameMap?.[scope.scope.row.outStockUserId] ?? "-" }}
                  </template>
                  ...
                  <template #outStockTime="scope">
                    {{ scope.scope.row.outStockTime ? new Date(scope.scope.row.outStockTime).toLocaleString().replace(/\//g, "-") : "-" }}
                  </template>
                  ...
                  <template #operate="scope">
                    <IconSvg
                      ...
                      v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.status === 0 && scope.scope.row.outStockStatus === 0"
                      @click="abandon(scope.scope.row)"
                    ></IconSvg>
                    <IconSvg
                      :src="StockIn"
                      class="fz20 m-r-20 pointer"
                      v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.status === 2 && scope.scope.row.outStockStatus === 0"
                      @click="stockOutDetail(scope.scope.row)"
                    ></IconSvg>
                    <el-icon
                      class="fz20 m-r-20 pointer"
                      v-if="hasPermission(PermissionType.Read, permissions) && scope.scope.row.status !== 0"
                      @click="showDetail(scope.scope.row)"
                    >
                      <View />
                    </el-icon>
                    <template v-if="scope.scope.row.status == 0">
                      <el-icon
                        class="fz20 m-r-20 pointer"
                        @click="edit(scope.scope.row)"
                        v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.status === 0"
                      >
                        <Edit />
                      </el-icon>
                    </template>
...
import { queryStockOutParams, getOutStockPage, stockOut, saveOutStock } from "@functions/serviceCenter/material-management/api/outstock";
...
import Create from "@functions/serviceCenter/material-management/stockOut/create.vue";
import StockIn from "@functions/serviceCenter/material-management/stockOut/stockOut.svg";
import IconSvg from "@static/components/iconSvg/iconSvg.vue";
import Abandon from "@functions/serviceCenter/material-management/stockOut/abandon.svg";
import { materialStockDetailSave } from "@functions/serviceCenter/material-management/api/materialStockDetail";
...
const businessStatusOptions = computed(() => {
  return [
    { id: -1, name: i18nM("material.enumValue.all") },
    { id: 0, name: i18nM("material.enumValue.notShipped") },
    { id: 1, name: i18nM("material.enumValue.shipped") },
...
const currentData = ref<stockOut | null>(null);
...
const columns = computed(() => [
  ...
  { prop: "stockId", label: i18nM("material.table.materialWarehouse") },
  { prop: "outStockNo", label: i18nM("material.table.warehouseOutNumber"), type: 2 },
  { prop: "outStockStatus", label: i18nM("material.label.businessStatus"), type: 2 },
  { prop: "status", label: i18nM("material.label.reviewStatus") },
  { prop: "outStockAmount", label: i18nM("material.table.stockOutAmount") },
  { prop: "outStockUserId", label: i18nM("material.table.outStockUser") },
  ...
  { prop: "approverCreateTime", label: i18nM("material.table.approvalTime") },
  { prop: "outStockTime", label: i18nM("material.table.outStockTime") },
...
const refreshTable = () => {
  ...
  const params: queryStockOutParams = {
  ...
  if (businessStatus.value > -1) params.outStockStatus = businessStatus.value;

  getOutStockPage(params, (res: any) => {
    const { content, totalElements } = res.data;
    const userIds: string[] = [];
    content?.forEach((event: any) => {
      userIds.push(event.outStockUserId);
...
const stockOutDetail = (row: any) => {
  ElMessageBox.confirm(i18nM("material.confirm.materialStockOut"), i18nM("material.title.materialStockOut"), {
    ...
    .then(() => {
      materialStockDetailSave({ subId: row.id, type: 2, projectId: projectId.value }, (res: any) => {
        if (res.code) {
          ElMessage({
            type: "success",
            message: i18nM("material.message.stockOutSuccess"),
          });
          refreshTable();
        }
      });
...
const abandon = (row: any) => {
  ElMessageBox.confirm(i18nM("material.confirm.abandon"), i18nM("material.title.stockOutNote"), {
    confirmButtonText: i18nM("material.button.confirm"),
    confirmButtonClass: "w-80",
    cancelButtonText: i18nM("material.button.cancel"),
    cancelButtonClass: "message-box-cancel-btn w-80",
    ...
    .then(() => {
      const params: any = { ...row, outStockStatus: 2 };
      delete params["files"];
      saveOutStock(params, (res: any) => {
        if (res.code) {
          ElMessage({
            type: "success",
