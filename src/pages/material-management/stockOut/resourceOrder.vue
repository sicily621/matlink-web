...
import { getInStockPage } from "@functions/serviceCenter/material-management/api/instock";
...
const currentChange = (row: any) => {
  ...
  resourceNo.value = props.type === 1 ? row.applyNo : row.inStockNo;
};
// 禁用日期
...
const columns = computed(() => {
  ...
  if (props.type === 1) {
    ...
    result = [
      { prop: "index", label: i18nM("material.table.index"), type: 1, width: "100%" },
      { prop: "inStockNo", label: i18nM("material.table.warehouseReceiptNumber") },
...
const refreshTable = () => {
  let params: any;
  let api = props.type === 1 ? materialOutBoundApplyFindPage : getInStockPage;
  ...
  if (props.type === 1) {
    ...
    if (name.value) params.inStockNo = name.value;
  }
  ...
  api(params, (res: any) => {
    ...
    content?.forEach((row: any) => {
      userIds.push(props.type === 1 ? row.applyUserId : row.inStockUserId);
    });
    ...
    usersGetNameMapByIds(newUserArr, (res: any) => {
      ...
      nextTick(() => {
        ...
        if (targetRow) {
          resourceNo.value = props.type === 1 ? targetRow.applyNo : targetRow.inStockNo;
