  ...
  <div class="d-flex h-full w-full">
    <div class="flex-1 d-flex h-full w-full" v-if="!processFlag">
      <div class="flex-1 flex-column h-full">
          ...
          <div class="d-flex flex-1 align-items-center justify-content-between">
            <div class="d-flex align-items-center">
              <div class="title d-flex w-150">
                ...
                <span>{{ i18nM("material.title.materialReceiptNote") }}</span>
              </div>
        ...
        <div class="flex-1 d-flex w-full content-wrap">
            ...
            <div class="zc-base p-r-20 p-l-20 flex-column flex-1">
              <div ref="tableRef" class="flex-1 table-wrap">
                <baseTable :columns="columns" :table-data="tableData" :indexMethod="indexMethod(currentPage, pageSize)">
                  ...
                  <template #inStockStatus="scope">
                    {{ findNameById(businessStatusOptions, scope.scope.row.inStockStatus) }}
                  </template>
                  ...
                  <template #operate="scope">
                    <IconSvg
                      ...
                      v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.status === 0 && scope.scope.row.inStockStatus === 0"
                      @click="abandon(scope.scope.row)"
                    ></IconSvg>
                    <IconSvg
                      :src="StockIn"
                      class="fz20 m-r-20 pointer"
                      v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.status === 2 && scope.scope.row.inStockStatus === 0"
                      @click="stockInDetail(scope.scope.row)"
                    ></IconSvg>
                    <el-icon
                      class="fz20 m-r-20 pointer"
...
import { queryStockInParams, stockIn, postInStockApprovedList, saveInStock } from "@functions/serviceCenter/material-management/api/instock";
...
import Create from "@functions/serviceCenter/material-management/stockIn/create.vue";
import Abandon from "@functions/serviceCenter/material-management/stockIn/abandon.svg";
...
const businessStatusOptions = computed(() => {
  return [
    { id: -1, name: i18nM("material.enumValue.all") },
    { id: 0, name: i18nM("material.enumValue.notReceived") },
    { id: 1, name: i18nM("material.enumValue.received") },
...
const currentData = ref<stockIn | null>(null);
...
const columns = computed(() => [
  ...
  { prop: "inStockNo", label: i18nM("material.table.warehouseReceiptNumber"), type: 2 },
  { prop: "inStockStatus", label: i18nM("material.label.businessStatus"), type: 2 },
  { prop: "status", label: i18nM("material.label.reviewStatus") },
  { prop: "inStockAmount", label: i18nM("material.table.amount") },
  { prop: "inStockUserId", label: i18nM("material.table.warehouser") },
  ...
  { prop: "inStockTime", label: i18nM("material.table.warehousingTime") },
...
const refreshTable = () => {
  ...
  const params: queryStockInParams = {
  ...
  if (businessStatus.value > -1) params.inStockStatus = businessStatus.value;

  postInStockApprovedList(params, (res: any) => {
...
const stockInDetail = (row: any) => {
  ElMessageBox.confirm(i18nM("material.confirm.stockIn"), i18nM("material.title.materialReceipt"), {
    ...
    .then(() => {
      materialStockDetailSave({ subId: row.id, type: 1, projectId: projectId.value }, (res: any) => {
        if (res.code) {
          ElMessage({
            type: "success",
            message: i18nM("material.message.stockInSuccess"),
          });
          refreshTable();
        }
      });
    ...
    .catch(() => {
      ElMessage({
        type: "info",
        message: i18nM("material.message.cancelStockIn"),
      });
    });
};
const create = () => {
...
const abandon = (row: any) => {
  ElMessageBox.confirm(i18nM("material.confirm.abandon"), i18nM("material.title.materialReceiptNote"), {
    confirmButtonText: i18nM("material.button.confirm"),
    confirmButtonClass: "w-80",
    cancelButtonText: i18nM("material.button.cancel"),
    cancelButtonClass: "message-box-cancel-btn w-80",
    ...
    .then(() => {
      const params: any = { ...row, inStockStatus: 2 };
      delete params["files"];
      saveInStock(params, (res: any) => {
        if (res.code) {
          ElMessage({
            type: "success",
