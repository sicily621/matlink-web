<template>
  <div class="d-flex h-full w-full">
                ...
                <baseTable :columns="columns" :table-data="tableData" :indexMethod="indexMethod(currentPage, pageSize)">
                  ...
                  <template #operate="scope">
                    ...
                    <IconSvg
                      ...
                      v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.status === 2 && scope.scope.row.taskStatus === 4"
                    ...
                    <template v-if="scope.scope.row.status == 0">
                      <el-icon
                        ...
                        v-if="hasPermission(PermissionType.Update, permissions) && scope.scope.row.taskStatus === 0"
                      >
                        <Edit />
                      </el-icon>
                      <el-icon
                        ...
                        v-if="hasPermission(PermissionType.Delete, permissions) && scope.scope.row.taskStatus === 0"
                      >
                        <Delete />
                      </el-icon>
                    </template>
    ...
    <div class="flex-1 flex-column h-full create-wrap" :class="{ 'fill-bg': processFlag != 2 }" v-if="processFlag">
      <Create ref="createRef" class="content-wrap" :data="currentData" :stockId="currentNodeKey" :projectId="projectId" v-if="processFlag === 1">
      </Create>
...
import { taskStockIn } from "@functions/serviceCenter/material-management/api/materialStockDetail";
...
import StockIn from "@functions/serviceCenter/material-management/task/stockIn.svg";
import Create from "@functions/serviceCenter/material-management/task/create.vue";
import Detail from "@functions/serviceCenter/material-management/task/detail.vue";
