  ...
  <div class="flex">
    <el-card class="width-200 h-full">
      ...
      >
        ...
        <div class="width-300" v-else>
          ...
          <el-form-item label="采购日期" prop="applyDate">
            <el-date-picker
              :disabled="onlyView"
    ...
    <div class="p-4 el-card flex-1 h-full m-l-3 flex flex-column">
      <div class="flex align-items-center justify-content-between">
        ...
        <div>
          <span v-if="errorShow" class="error m-r-8">采购详情不能为空</span>
      ...
      <div class="flex-1 table-wrap" ref="tableWrapRef">
        ...
        >
          ...
          <template #supplierId="scope">
            <el-select
              v-if="editSupplyRow?.id == scope.scope.row.id"
              class="w-30"
...
const materialIds = ref<string[]>(
  props.checkmaterialIds ? props.checkmaterialIds.split(",") : [],
);
...
const inputChange = (row: any) => {
  row.totalPrice = Number(Number(+row.perPrice * +row.count).toFixed(2));
};
const validateBillNoFn = async (rule: any, value: any, callback: any) => {
  if (value == "") return callback(new Error("不能为空"));
  const params: any = { billNo: form.value.billNo };
  if (form.value.id) params.id = form.value.id;
  const res: any = await validateBillNo(params);
  if (!res.data) {
    callback(new Error("采购单号不能重复"));
  } else {
    callback();
  }
};
const rules = reactive<FormRules>({
  billNo: [{ validator: validateBillNoFn as any, trigger: "blur" }],
  stockId: [{ required: true, message: "不能为空" }],
  applyDate: [{ required: true, message: "不能为空" }],
...
const columns = computed(() => {
  const result: any[] = [
    ...
    { prop: "brand", label: "品牌" },
    { prop: "unit", label: "单位" },
    { prop: "count", label: "采购数量", type: props.onlyView ? 0 : 8 },
    { prop: "perPrice", label: "单价", type: props.onlyView ? 0 : 8 },
    { prop: "totalPrice", label: "总价" },
    { prop: "supplierId", label: "供应商", type: props.onlyView ? 0 : 8 },
...
const disabledHours = () => {
  if (!form.value.purchaseTime) return [];

  const selectedDate = new Date(form.value.purchaseTime);
...
const queryStockOptions = async () => {
  ...
  if ((res as any)?.data?.length) {
    ...
    if (props.stockId) form.value.stockId = props.stockId;
  }
};
...
const confirmSave = async (cb?: Function) => {
  try {
    ...
    const hasZeroCount = tableData.value.some((item: any) => item.count == 0);
    if (hasZeroCount) {
      ElMessage({
        type: "error",
        message: "采购数量不能为0",
    ...
    if (valid) {
      ...
      const params: any = {
        ...form.value,
        applyDate:
          typeof form.value.applyDate === "string"
            ? new Date(form.value.applyDate).getTime()
            : form.value.applyDate.getTime(),
      };
      ...
      const api = params.id ? editPurchase : createPurchase;
      const res: any = await api(params);
      await deletePurchaseDetail(res.data.id);
      const details = tableData.value.map((item: any) => {
        const { materialId, count, perPrice, totalPrice, supplierId } = item;
        const result: any = {
          billId: res.data.id,
          materialId,
      ...
      await savePurchaseDetail(details);
      ElMessage({
        type: "success",
...
const refreshTable = async () => {
  ...
  tableData.value = res.data.map((item: any) => {
    ...
    const row: any = {
      count: 0,
      perPrice: item?.perPrice ?? material?.costPrice ?? 0,
      totalPrice: 0,
      supplierId: 0,
    };
