...
import { getStockList, Stock } from "@pages/baseManagement/api/stock";
...
const confirmSave = () => {
  ...
  router.push({
    ...
    query: {
      checkedIds: checkedIds.value.join(","),
      stockId: stockCurrentNodeKey.value,
    },
  });
};
...
function refreshTable() {
  ...
  findMaterialPage(params)
    .then(async (res: any) => {
      ...
      materialIds.value = list.map((item: any) => item.id);
      if (materialIds.value.length) await queryMaterial();
      if (unitIds.length) await getUnitList(unitIds);
      totalItems.value = total;
      tableData.value = list.map((item: any) => {
        return {
          ...item,
          count: materialMap.value.get(item.id)?.count || 0,
        };
      });
      nextTick(() => {
