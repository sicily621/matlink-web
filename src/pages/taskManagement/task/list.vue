                  ...
                  <div class="flex">
                    ...
                    <IconSvg
                      v-if="
                        enableEdit &&
                        scope.scope.row.auditStatus === 0 &&
                        scope.scope.row.status <= TaskStatus.Pending 
                      "
                      :src="TaskPending"
                      :width="16"
                      class="fz16 m-r-5 cursor-pointer"
                      @click="toInputResult(scope.scope.row)"
                    ></IconSvg>
                    <IconSvg
                      v-if="
                        enableEdit &&
                      ...
                      :width="16"
                      class="fz16 m-r-5 cursor-pointer"
                      @click="toFinish(scope.scope.row)"
                    ></IconSvg>
                  </div>
                </template>
              </baseTable>
...
import Abandon from "@pages/taskManagement/task/abandon.svg";
import IconSvg from "@static/components/iconSvg/iconSvg.vue";
import Finish from "@pages/taskManagement/task/finish.svg";
import TaskPending from "@pages/taskManagement/task/taskPending.svg";
...
const resultEntry = ref(false);
const toInputResult = (row: any) => {
  resultEntry.value = true;
  currentData.value = row;
  processFlag.value = 1;
};
const toFinish = (row: any)=>{
  ElMessageBox.confirm("确定要结束盘点吗？", "盘点任务", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
    .then(async () => {
      const params: any = { ...row, status: TaskStatus.Finished };
      const res: any = await editTask(params);
      if (res.code) {
        ElMessage({
          type: "success",
