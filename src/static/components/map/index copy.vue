<template>
  <div class="h-full relative" @click="click" @mousedown="mousedown" @mouseup="mouseup" @mousewheel="wheel" ref="mapRef">
    <TresCanvas renderMode="manual" :toneMappingExposure="1" :toneMapping="NoToneMapping" :antialias="true" :alpha="true" :logarithmicDepthBuffer="true" powerPreference="high-performance" ref="canvasRef">
      ...
      <TresHemisphereLight :intensity="mode ? 0 : 2" />
      <TresAmbientLight :intensity="1" />
      <TresDirectionalLight :intensity="mode ? 0 : 0.2" :position="[100, 0, 100]" />
      <TresDirectionalLight :intensity="mode ? 0 : 0.2" :position="[-100, 0, 100]" />
      <TresDirectionalLight :intensity="mode ? 0 : 0.2" :position="[100, 0, -100]" />
      <TresDirectionalLight :intensity="mode ? 0.5 : 0" :position="[5, 30, -3]" />
      <TresDirectionalLight ref="directionLightRef" :intensity="mode ? 0.5 : 5" />
      ...
      <TresObject3D ref="roadRootRef">
        <Road v-for="(road, index) in roadList" :data="road" :roadLabelShowIndex="roadLabelShowIndex" :key="road.key" :status="roamingStatus" :roamingRoadIndex="currentRoadIndex" :currentIndex="index" :setRoamingCamera="setRoamingCamera" :onReachRoamingPoint="onReachRoamingPoint" :roamingSpeed="roamingSpeed"> </Road>
      </TresObject3D>
      <labelRender ref="labelRef" :width="width" :height="height" :mapRef="mapRef" :camera="currentCamera" />
      <bloomPass :materials="materials" :mode="mode" :camera="currentCamera" />
    </TresCanvas>

    <div class="compass flex-column flex-center" ref="compassRef" v-if="!onlySky && !bindPointMode && props.compass && roamingStatus === RoamingStatus.STOP">
...
import { type Object3D, MOUSE, BoxHelper, Vector3, type Sphere, GridHelper, Raycaster, Vector2, Material, MeshBasicMaterial, MeshPhongMaterial, CubeTextureLoader, ShaderMaterial, BoxGeometry, Mesh, BackSide, ShaderLib, CubeRefractionMapping, AnimationMixer, Layers, TextureLoader, CanvasTexture, RepeatWrapping, AdditiveBlending, MeshPhysicalMaterial, PMREMGenerator, ACESFilmicToneMapping, SRGBColorSpace, BasicShadowMap, NoToneMapping } from "three";
...
watch(mode, () => {
  ...
  console.log("roamingOrbitControlsRef.value.enabled", roamingOrbitControlsRef.value.enabled);
  //   console.log("roamingOrbitControlsRef.value.enabled", roamingOrbitControlsRef.value.enabled);
});
// 添加模型到场景
const addToScene = (model: Object3D, needRefreshCamera: boolean = true) => {
...
const cameraAdjustment = () => {
  ...
  if (props.bindPointMode) {
    ...
    cameraPosition.value = new Vector3(lookAt.x - radius / 2.0 + model.position.x, radius * 1 + lookAt.y + model.position.y, lookAt.z + radius / 2.0 + model.position.z);
  }
