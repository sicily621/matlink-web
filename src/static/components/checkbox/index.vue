...
const name = ref("空调模式");
const id = 1;
const changeModelStatus = (val: boolean) => {};
</script>
...
.neon-checkbox {
  ...
  --primary-dark: var(--el-color-primary);
  --primary-light: var(--el-color-primary);
  --size: zrem(18);
  position: relative;
...
.neon-checkbox__box {
  ...
  border: zrem(1) solid var(--primary-dark);
...
.neon-checkbox__particles span {
  ...
  top: 50%;
  left: 50%;
  box-shadow: 0 0 zrem(6) var(--primary);
}
...
.neon-checkbox__rings .ring {
  ...
  inset: 0;
  border-radius: 50%;
  border: zrem(1) solid var(--primary);
  opacity: 0;
  transform: scale(0);
}
...
@keyframes sparkFlash {
  ...
  100% {
    transform: rotate(var(--r, 0deg)) translateX(zrem(30)) scale(0);
    opacity: 0;
  }
}
